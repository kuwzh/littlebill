...
def get_ventes_client(client_id: int, page: int = Query(1)):
    # filtre les ventes pour le client
    ventes_client = [v for v in ventes_data if v["client_id"] == client_id]
    
    # pagination
    start = (page - 1) * PAGE_LIMIT
    end = start + PAGE_LIMIT
    return {"page": page, "total": len(ventes_client), "ventes": ventes_client[start:end]}
